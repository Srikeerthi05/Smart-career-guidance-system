...
# Optionally encode hobbies or just ignore it if unused for prediction

   

    # Create feature array
    feature_array = np.array([[gender_encoded,  , absence_days, extracurricular_activities_encoded,
                               weekly_self_study_hours, math_score, history_score, physics_score,
    ...
    # Map predictions to class names with probabilities
    predictions = [(class_names[idx], probabilities[0][idx] * 100) for idx in np.argsort(-probabilities[0])]

    # Filter out 'Unknown'
    filtered = [(name, prob) for name, prob in predictions if name.lower() != 'unknown']

    # Return top 3 (or fewer if limited)
    return filtered[:3]
